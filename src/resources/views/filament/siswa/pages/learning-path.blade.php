<x-filament-panels::page>
    <div class="space-y-6">
        ...
        @if ($student && $student->stream)
            <div class="rounded-2xl p-6 text-white"
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <p class="text-blue-200 text-sm font-medium uppercase tracking-wider mb-1">Learning Pathway</p>
                        <h1 class="text-2xl font-bold">{{ $student->stream->nama }}</h1>
                        @if ($student->stream->deskripsi)
                            <p class="text-blue-100 mt-1 text-sm">{{ $student->stream->deskripsi }}</p>
                        @endif
                    </div>
        ...
        @else
            <div class="rounded-2xl p-6 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-center">
                ...
                <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">
                    Hubungi wali kelas atau admin untuk menetapkan Learning Pathway kamu.
                </p>
            </div>
        @endif
                ...
                @foreach ($student->stream->milestones->where('is_aktif', true) as $milestone)
                    ...
                    <div class="relative flex gap-5 group">
                        ...
                        <div class="flex-1 rounded-xl border p-5 mb-1 transition-all duration-200
                            ...
                            {{-- Tips box — only current --}}
                            @if ($isCurrent && $milestone->tips)
                                <div class="mt-4 p-3 rounded-lg bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800">
                                    <p class="text-xs font-semibold text-blue-700 dark:text-blue-300 uppercase tracking-wider mb-1">
                                        💡 Tips Semester Ini
                                    </p>
        ...
        @elseif ($student && $student->stream)
            <div class="text-center py-10 text-gray-400">
                <x-heroicon-o-cube class="w-10 h-10 mx-auto mb-2" />
                <p class="text-sm">Milestone untuk stream ini belum dikonfigurasi.</p>
            </div>
        @endif
